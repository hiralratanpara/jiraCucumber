...
describe('wsi ship to store ', () => {
    ...
    it('Verify the BOSTS overlay opens on change store click', () => {
        ...
        cy.get(findAStorePopUp.findAStorePopupTxt).should('have.text', findAStorePopUp.findAStoreTxt);
        cy.get(findAStorePopUp.closeBtn).click();
    });
...
*/
    it('Verify that BOSTS popup should open when "item Not available near by" and STS option is selected', () => {
        ...
        if(cy.get(productPage.notAvailableNearTxt).contains(productPage.notAvailableText).should('be.visible')){
            cy.get(productPage.shipToStoreBtn).click();
            cy.get(findAStorePopUp.findAStorePopupTx).should('have.text', findAStorePopUp.FindAStoreTxt);
        }else{
